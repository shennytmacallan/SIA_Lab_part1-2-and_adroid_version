...
postsContainer.innerHTML = '<div class="alert alertdanger">Failed to load posts. Please refresh.</div>';
} finally {
loading = false;
...
<button class="action-btn vote-btn" data-post-id="${post.id}" data-vote="1"><i class="bi bi-arrowup"></i>
 <span class="vote-count up-${post.id}">${post.upvotes}</span></button>
<button class="action-btn vote-btn" data-post-id="${post.id}" data-vote="-1"><i class="bi bi-arrowdown"></i> <span class="vote-count down-${post.id}">${post.downvotes}</span></button>
<a href="<?php echo BASE_URL;?>/post.php?id=${post.id}" class="action-btn comment-link"><i
class="bi bi-chat"></i> <span class="votecount">${post.comments}</span></a>
</div>
<div class="post-footer">Posted by <a
href="<?php echo BASE_URL;?>/profile.php?id=${post.user.id}">@${post.user.username}</a><
/div>
</div>
`;
...
try {
const response = await fetch('<?php echo BASE_URL;?>/api/vote.php', {method: 'POST',headers: { 'Content-Type': 'application/json' },body: JSON.stringify({ post_id: postId, vote: vote})});
